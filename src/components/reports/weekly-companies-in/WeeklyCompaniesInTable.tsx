...
export type WeeklyCompanyInItem = {
  id: string;
  companyName: string;
  operatorName: string;
  postCode: string;
...
type Props = {
  items: WeeklyCompanyInItem[];
  onOpenDetails: (item: WeeklyCompanyInItem) => void;
};

const WeeklyCompaniesInTable: React.FC<Props> = ({ items, onOpenDetails }) => {
  if (!items.length) {
    return (
        ...
        <table className="min-w-[820px] w-full text-sm">
          <thead className="bg-gray-50">
            <tr className="text-left text-[11px] text-gray-500">
              ...
              <th className="px-5 py-3">Jobs IN</th>
          ...
          <tbody>
            {items.map((it) => (
              ...
              >
                ...
                <td className="px-5 py-4">{it.jobsIn}</td>
...
export default WeeklyCompaniesInTable;
